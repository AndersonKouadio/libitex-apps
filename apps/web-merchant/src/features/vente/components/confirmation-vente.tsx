        ...
        <Modal.Dialog>
          <Modal.Body className="text-center py-8">
            ...
            <p className="text-3xl font-bold text-accent mt-4 tabular-nums">
              {formatMontant(total)}
              <span className="text-base font-normal text-muted ml-1">F CFA</span>
            </p>
            {monnaie > 0 && (
              <p className="text-sm text-warning mt-2 tabular-nums">
                Monnaie a rendre : {formatMontant(monnaie)} F
              </p>
            )}
          </Modal.Body>
          <Modal.Footer>
            <Button variant="primary" className="w-full" onPress={onNouvelle}>
              Nouvelle vente
            </Button>
          </Modal.Footer>
        </Modal.Dialog>
